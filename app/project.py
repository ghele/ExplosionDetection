...
def test_message(message):
    ...
    thread = Thread(target=explosion_simulation, kwargs={'xAxis':message['xAxis'], 'yAxis':message['yAxis'], 'intensity': message['intensity']})
    thread.start()

    print("explosion on", message['xAxis'], message['yAxis'], message['intensity'])

@sio.on('connect', namespace='/socket')
def test_connect():
    emit('notification', {'data': 'Connected'})

def explosion_simulation(xAxis=0, yAxis=0, intensity=0):
    notificationOrder = orderedSensors(sensors, float(xAxis), float(yAxis), intensity)
...
if __name__ == '__main__':
    # load JSON file containing the bidimensional coordinatates of each sensor
